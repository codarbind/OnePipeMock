...
//data validation
let {auth_provider, customer_ref, amount, firstname, surname, middlename, email, mobile_no, start_date, end_date} = req.body

if(!(auth_provider && customer_ref && amount && firstname && surname && email && mobile_no && start_date && end_date)) return res.send({message:'supply all compulsory inputs'})
...
.catch(function (error) {
  ...
  res.status(400).json({message:'there was an error'})
	...
	console.log('statement of account route')
